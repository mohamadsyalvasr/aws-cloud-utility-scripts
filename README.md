...
- **`eks_report.sh`**: Generates a detailed report on all EKS clusters.
- **`elasticache_report.sh`**: Generates a detailed report on all ElastiCache clusters.
- **`elb_report.sh`**: Generates a detailed report on all Elastic Load Balancers (ELBv2: ALB/NLB/GWLB) across regions.
...
2.  **Make the Main Script Executable**:
    You only need to do this once. The main script will handle setting permissions for the other scripts.
    ```bash
...
| `-s` | Enables the summation of all attached EBS volumes. (Only applies to scripts that support it). |
