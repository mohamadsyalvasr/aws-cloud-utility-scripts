#!/bin/bash
# aws_rds_metrics_graph.sh
# Fetches time-series CloudWatch metrics (CPU, Freeable Memory) for RDS instances
...
FILENAME="${OUTPUT_DIR}/aws_rds_metrics_data.json"
START_DATE=""
END_DATE=""
...
Options:
  ...
  -f <filename>    Name of the output JSON file.
                   Default: aws_rds_metrics_data_<timestamp>.json
...
check_dependencies
mkdir -p "$OUTPUT_DIR"
log "✍️ Preparing output file for RDS: $FILENAME"
...
for region in "${REGIONS[@]}"; do
    ...
    log "  [RDS] Fetching metrics for RDS instances..."
    while IFS= read -r db_instance; do
        ID=$(echo "$db_instance" | jq -r '.DBInstanceIdentifier')
        # Fetch Name tag
        DB_ARN=$(echo "$db_instance" | jq -r '.DBInstanceArn')
        NAME=$(aws rds list-tags-for-resource --resource-name "$DB_ARN" --region "$region" --query 'TagList[?Key==`Name`].Value' --output text 2>/dev/null | tr -d '\n' || echo "N/A")
        NAME=${NAME:-"N/A"}

        # 1. CPU Utilization (AWS/RDS)
        CPU_METRICS=$(fetch_metric_data "$ID" "RDS" "$region" "AWS/RDS" "CPUUtilization" "DBInstanceIdentifier" "$NAME")
        ...
        # 2. Freeable Memory (AWS/RDS)
        MEM_METRICS=$(fetch_metric_data "$ID" "RDS" "$region" "AWS/RDS" "FreeableMemory" "DBInstanceIdentifier" "$NAME")
...
log "✅ DONE. RDS time series metrics data saved to: $FILENAME"